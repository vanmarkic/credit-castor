...
export default function FoundersToQuotiteTransition() {
  ...
  useEffect(() => {
    ...
    // Calculate positions for pie chart arrangement (circle around center)
    const centerX = window.innerWidth / 2;
    const centerY = window.innerHeight / 2;
    const radius = 0; // They'll merge to center

    founderRefs.current.forEach((founder, index) => {
      if (!founder) return;

      const angle = -90 + (founders.slice(0, index).reduce((sum, f) => sum + f.percentage, 0) / 100 * 360);
  ...
  return (
    ...
    >
      <div
        ref={containerRef}
        className="fixed inset-0 flex items-center justify-center bg-gradient-to-b from-slate-900 to-slate-950"
      >
        ...
        <div className="absolute bottom-20 left-0 right-0 text-center">
          <p className="text-2xl text-slate-300">
            De <span className="text-blue-400">4 fondateurs</span> à leur <span className="text-purple-400">quotité</span>
          </p>
        </div>
      </div>
    </section>
