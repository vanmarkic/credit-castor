...
export default function CoproSaleDistributionView({ sale }: CoproSaleDistributionViewProps) {
  ...
  const totalBuildingSurface = Array.from(pricing.distribution.toParticipants.keys()).reduce((sum, founderName) => {
    // Calculate total surface from distribution (reverse engineer from amounts)
    return sum;
  }, 0);
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-lg p-6" data-testid="copro-sale-distribution">
      ...
      {/* Distribution: 30/70 Split */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {/* 30% to Copropriété Reserves */}
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {Array.from(pricing.distribution.toParticipants.entries()).map(([founderName, amount]) => {
                // Calculate quotité from amount and total
                const quotite = (amount / (pricing.totalPrice * 0.7)) * 100;
            ...
            <tfoot className="bg-gray-50 border-t-2 border-gray-300">
              <tr>
                ...
                <td className="p-3 text-right font-bold text-gray-700">
                  €{Array.from(pricing.distribution.toParticipants.values())
      ...
      <div className="mt-4 p-3 bg-amber-50 border border-amber-200 rounded text-sm text-amber-800">
        <strong>Note:</strong> The 70% distribution is split among founders according to their
