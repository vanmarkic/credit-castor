...
export default function TimelineVisualization({
  ...
  const totalMonths = monthsBetween(deedDate, latestDate);

  // Calculate position for each event
  const eventPositions = events.map(event => ({
    event,
    monthsFromDeed: monthsBetween(deedDate, event.date),
    percentage: (monthsBetween(deedDate, event.date) / totalMonths) * 100,
  }));

  // Current position
  const nowPercentage = (monthsBetween(deedDate, currentDate) / totalMonths) * 100;
